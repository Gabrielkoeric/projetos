...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCart(index, quantity) {
    var estoqueId = increaseButtons[index].getAttribute('data-estoque-id');
    var productIndex = cart.findIndex(function (item) {
      return item.estoqueId === estoqueId;
    });
  ...
  finalizarPedidoButton.addEventListener('click', function () {
    if (cart.length > 0) {
      ...
      formData.append('cart', JSON.stringify(cart)); // Construir a URL de forma convencional

      var url = '/compra/store'; // Substitua pelo caminho real da rota
      ...
      }).then(function (data) {// Lógica para lidar com a resposta do servidor, se necessário
